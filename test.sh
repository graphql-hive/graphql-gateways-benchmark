...
if [[ -n "${MEM_LIMIT:-}" ]]; then
  ...
  if ! command -v cgexec >/dev/null; then
    echo "WARN: cgexec not found; cannot apply memory limit of $MEM_LIMIT" >&2
  fi
fi
...
if [[ "${CI:-}" == "true" ]]; then
  ...
  npx --quiet capture-website-cli "http://localhost:3000/d/01npcT44k/k6?orgId=1&from=${START_TIME}000&to=${END_TIME}000&kiosk" --output $GATEWAY_DIR/overview.png --width 1200 --height 900

  rm -rf $GATEWAY_DIR/http.png || echo ""
  npx --quiet capture-website-cli "http://localhost:3000/d-solo/01npcT44k/k6?orgId=1&from=${START_TIME}000&to=${END_TIME}000&panelId=41" --output $GATEWAY_DIR/http.png --width 1200 --height 900
